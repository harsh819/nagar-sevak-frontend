      ...
      <div className="container">
        ...
        <div className="grid lg:grid-cols-2 gap-10 lg:gap-16 items-center mb-16">
          <div>
            ...
            <p className="text-lg text-muted-foreground mb-6 leading-relaxed">
              जनतेच्या सेवेसाठी समर्पित। Our Nagar Sevak has been at the forefront of transforming {office?.wardName || "the ward"} into
          ...
          <div className="relative">
            <div className="aspect-[4/5] rounded-2xl overflow-hidden shadow-2xl bg-slate-100 flex items-center justify-center">
              {office?.profilePic ? (
                <img
                  src={office.profilePic}
                  alt={office.nagarSevakName}
                  className="w-full h-full object-cover object-top"
                />
              ) : (
                <span className="text-6xl font-black text-slate-300">{office?.nagarSevakName?.substring(0, 1)}</span>
              )}
            </div>
            <div className="absolute -bottom-6 -left-6 bg-accent text-accent-foreground rounded-xl p-6 shadow-lg">
              <p className="text-3xl font-bold">{office?.wardName || "Ward Office"}</p>
              <p className="text-sm opacity-90">Legislative Council Office</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-secondary rounded-2xl p-8 lg:p-12">
          <div className="max-w-3xl mx-auto text-center">
            <div className="w-20 h-20 rounded-full overflow-hidden mx-auto mb-6 border-4 border-accent bg-slate-200 flex items-center justify-center">
              {office?.profilePic ? (
                <img
                  src={office.profilePic}
                  alt={office.nagarSevakName}
                ...
                <span className="text-2xl font-black text-slate-400">{office?.nagarSevakName?.substring(0, 1)}</span>
              )}
            </div>
